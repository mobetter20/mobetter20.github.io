...
# In-universe: the Coo's first published volume was 2015 (Vol I).
# 2014 is the founding/pre-publication year (Vol 0, never appears in print).
_COO_VOLUME_EPOCH = 2014
...
def coo_edition_label(issue: Issue) -> str:
    season = _SEASON_BY_MONTH[issue.issue_date.month]
    year = issue.issue_date.year
    volume = _to_roman(max(1, year - _COO_VOLUME_EPOCH))
    no = issue.issue_number.lstrip("0") or "0"
    return f"Vol. {volume} · No. {no} · {season} {year}"


def render_avian_district_excerpt(issue: Issue) -> str:
    headline = html.escape(issue.lead_headline, quote=True)
    edition = html.escape(coo_edition_label(issue), quote=True)
    return (
        ...
        f'      <span class="coo-edition">{edition}</span>\n'
        '    </div>\n'
