...
Gate 3 (the roster scale-up) is built: all 16 cards real with lore backs,
scopes frozen per city (D25), three switchable stat themes (CORE /
SERVICE / MONEY, D24/D26), battle and daily across the full deck. The
owner verdict on the gate-3 page decides what's next; soft-launch norms
hold until that separate owner call.
...
`is/building/world-metros/index.html` is GENERATED by
...
`build_page_geometry.py`) plus all sixteen ledger diagrams
